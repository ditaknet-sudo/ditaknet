...
def create_backup(name: str | None = None) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix=".ditaknet-db-backup-", dir=target.parent) as tmp:
...
async def create_full_backup(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=".ditaknet-full-backup-", dir=target.parent) as tmp:
        tmp_path = Path(tmp)
    ...
    if path.suffix.lower() == ZIP_SUFFIX:
        ...
        with zipfile.ZipFile(path) as zf:
            ...
            if "backup.json" in names:
                try:
                    manifest = _validate_backup_manifest(json.loads(zf.read("backup.json")))
                except json.JSONDecodeError as exc:
                    raise ValueError("Backup manifest is not valid JSON") from exc
            with tempfile.TemporaryDirectory(prefix="ditaknet-validate-backup-") as tmp:
                extracted = Path(tmp) / "database.sqlite3"
                with zf.open("database.sqlite3") as source, extracted.open("wb") as target:
