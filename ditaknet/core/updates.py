...
def _pre_key(parts: tuple[str, ...]) -> tuple:
    ...
    if not parts:
        return ((1, 0),)  # release beats pre-release
    out: list[tuple] = []
    for part in parts:
        if part.isdigit():
...
def verify_manifest_signature(raw_body: bytes, signature: str | None) -> bool:
    ...
    Signature may be hex or base64 of HMAC over the raw JSON body.
    """
    ...
    try:
        ...
        if not re.fullmatch(r"[0-9a-f]{64}", candidate):
            as_hex = base64.b64decode(signature.strip()).hex()
        return hmac.compare_digest(expected, as_hex)
    except Exception:
