(function () {
  ...
  // Session-authenticated mutation APIs require the same CSRF token as HTML
  // forms. Wrap same-origin fetch calls once so page-specific scripts cannot
  // accidentally omit the header. Bearer-token/external clients are unchanged.
  var nativeFetch = window.fetch.bind(window);
  window.fetch = function (input, init) {
    var options = Object.assign({}, init || {});
    var requestMethod = options.method || (input instanceof Request ? input.method : "GET");
    var method = String(requestMethod || "GET").toUpperCase();
    ...
    var token = document.body && document.body.getAttribute("data-csrf");
    if (unsafe && token && target.origin === window.location.origin) {
